...
void PortalFileDialog::open()
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this] (QDBusPendingCallWatcher *watcher) {
        ...
        if (!reply.isError()) {
            ...
            qWarning() << "Failed to open portal file dialog: " << reply.error().message();
        }
    });
#endif
}
...
void PortalFileDialog::gotResponse(uint response, const QVariantMap &results)
{
    if (!response) {
        ...
        qDebug() << "Portal file dialog was cancelled";
    } else {
        qWarning() << "Portal file dialog failed with response code:" << response;
    }
}
